...
const registerUser = asyncHandler(async (req, res) => {
   ...
   // uplode then to cloudinary, avtar
   ...
   const avatar = await uploadPOnCloudinary(avatarLocalPath)
   const coverImage = await uploadPOnCloudinary(coverImageLocalPath)
   if (!avatar) {
      throw new ApiError(400, "Avatar file is required")
   }
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
   ...
   return res
      .status(200)
      .json(new ApiResponse(200, "password chanded successfully"))
        ...
        },
        {
            $addFields: {
               ...
               isSubscribed: {
                  $cond: {
                     if: {
                        $sin: [req.user?._id, "$subscribers.subscriber"]
                     },
                     then: true,
                     else: false
                  }
...
const getWathchHistory = asyncHandler(async (req, res) => {
